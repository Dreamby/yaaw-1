...
if (typeof ARIA2=="undefined"||!ARIA2) var ARIA2=(function(){
  ...
  return {
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "WebSocket 错误. 请刷新后重试.");
          ws_callback = {};
        };
        ...
        main_alert("alert-error", "错误的协议类型");
      };
    },
    ...
    restart_task: function(gids) {
      ...
      if (uris.length == 0) {
        main_alert("alert-error", "找不到文件，BT 任务无法恢复!", 2000);
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          if (select_lock) return;
          if (!result.result) {
            main_alert("alert-error", "<strong>错误: </strong>RPC 回调数据有误.", 5000);
          }
    ...
    change_pos: function(gid, pos, how) {
      ARIA2.request("changePosition", [gid, pos, how],
        function(result) {
          ...
          main_alert("alert-info", "移动任务成功", 1000);
          ARIA2.refresh();
        }
      );
    },
    ...
    pause: function(gids) {
      ...
      ARIA2.batch_request("pause", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "任务已暂停", 1000);
            ARIA2.refresh();
          } else {
    ...
    unpause: function(gids) {
      ...
      ARIA2.batch_request("unpause", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "任务已启动", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    change_options: function(gid, options) {
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          ...
          main_alert("alert-info", "设置已保存", 1000);
        }
      );
    },
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已经暂停所有任务. (BT 任务稍后将自动告知 Tracker)", 2000);
        }
      );
    },
    ...
    unpause_all: function() {
      ARIA2.request("unpauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已经启动所有任务.", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "成功清理已下载/错误/已删除任务.", 2000);
        }
      );
    },
    ...
    get_global_option: function() {
      ARIA2.request("getGlobalOption", [],
        function(result) {
          if (!result.result)
            main_alert("alert-error", "<strong>错误: </strong>RPC 返回数据有误.", 5000);
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "修改设置成功!", 2000);
          }
        }
      );
    },
